...
if ( ! function_exists( 'cps_entry_footer' ) ) :
	...
	function cps_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'spin' ) );
			if ( $categories_list && cps_categorized_blog() ) {
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'spin' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
...
function cps_get_svg( $args = array() ) {
	...
	if ( empty( $args ) ) {
		return esc_html__( 'Please define default parameters in the form of an array.', 'spin' );
	}

	// YUNO define an icon?
	if ( false === array_key_exists( 'icon', $args ) ) {
		return esc_html__( 'Please define an SVG icon filename.', 'spin' );
	}
 ...
 */
function cps_do_build_text() {

	// Echo the text.
	echo '<div class="dev-info">' . esc_html( 'Built with ', 'spin' ) . '<span class="heart">&#9829; </span>' . esc_html( 'by ', 'spin' ) . '<a class="dev-link" href="' . esc_url( 'http://www.allisontarr.com', 'spin' ) . '">Allison Tarr</a></div>'; // WPCS: XSS OK.
}
...
function cps_get_social_share() {
	...
	<div class="social-share">
		<h5 class="social-share-title"><?php esc_html_e( 'Share This', 'spin' ); ?></h5>
		<ul class="social-icons menu menu-horizontal">
			<li class="social-icon">
				<a href="<?php echo esc_url( $twitter_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=600, height=300' ); return false;">
					<?php cps_do_svg( array( 'icon' => 'twitter-square', 'title' => 'Twitter', 'desc' => __( 'Share on Twitter', 'spin' ) ) ); ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on Twitter', 'spin' ); ?></span>
				</a>
			</li>
			<li class="social-icon">
				<a href="<?php echo esc_url( $facebook_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=600, height=300' ); return false;">
					<?php cps_do_svg( array( 'icon' => 'facebook-square', 'title' => 'Facebook', 'desc' => __( 'Share on Facebook', 'spin' ) ) ); ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on Facebook', 'spin' ); ?></span>
				</a>
			</li>
			<li class="social-icon">
				<a href="<?php echo esc_url( $linkedin_url ); ?>" onclick="window.open(this.href, 'targetWindow', 'toolbar=no, location=no, status=no, menubar=no, scrollbars=yes, resizable=yes, top=150, left=0, width=475, height=505' ); return false;">
					<?php cps_do_svg( array( 'icon' => 'linkedin-square', 'title' => 'LinkedIn', 'desc' => __( 'Share on LinkedIn', 'spin' ) ) ); ?>
					<span class="screen-reader-text"><?php esc_html_e( 'Share on LinkedIn', 'spin' ); ?></span>
				</a>
			</li>
		</ul>
	</div><!-- .social-share -->
 ...
 */
function cps_do_social_share() {
	echo cps_get_social_share(); // WPCS: XSS OK.
}
...
?>
	<nav id="mobile-menu" class="mobile-nav-menu">
		<button class="close-mobile-menu"><span class="screen-reader-text"><?php _e( 'Close menu', 'spin' ); ?></span><?php cps_do_svg( array( 'icon' => 'close' ) ); ?></button>
		<?php
			wp_nav_menu( array(
...
function cps_get_social_links() {
	...
	<?php foreach ( $social_networks as $network ) :
		...
		$link_value = get_theme_mod( 'cps_' . $network . '_link' ); ?>
