...
function cps_customize_register( $wp_customize ) {
	...
	$wp_customize->add_section(
		...
		array(
			'title'       => esc_html__( 'Social Links', 'spin' ),
			'description' => esc_html__( 'These are the settings for social links. Please limit the number of social links to 3.', 'spin' ),
			'priority'    => 90,
		)
	);
	...
	foreach ( $social_networks as $network ) {
		...
		$wp_customize->add_control(
			'cps_' . $network . '_link',
			array(
				'label'   => sprintf( esc_html__( '%s Link', 'spin' ), ucwords( $network ) ),
				'section' => 'cps_social_links_section',
				'type'    => 'text',
			)
		);
	...
	$wp_customize->add_section(
		'cps_footer_section',
		array(
			'title'    => esc_html__( 'Footer Customization', 'spin' ),
			'priority' => 90,
		)
	);
	...
	$wp_customize->add_setting(
		...
		array(
			'default' => '',
			'sanitize_callback' => 'cps_sanitize_customizer_text',
		)
	);
	$wp_customize->add_control(
		'cps_copyright_text',
		array(
			'label'       => esc_html__( 'Copyright Text', 'spin' ),
			'description' => esc_html__( 'The copyright text will be displayed beneath the menu in the footer.', 'spin' ),
